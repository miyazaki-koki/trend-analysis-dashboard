...
### Step 4: Codex CLI Quality Check (External AI Review)

Execute design quality check using Codex CLI for additional perspective.
...
Run `git diff --name-only develop` and check if `.kiro/specs/$1/design.md` is included.
(Note: After generating design.md in Step 3, changes should exist)
...
- **If design.md has no changes** → Invoke `/codex-exec` skill with quality check prompt
...
For codex-exec (fallback when no changes):
- Skill name: `codex-exec`
- Arguments: `"Review this technical design document for quality. Check: Completeness, Clarity, Consistency, Type Safety, Feasibility. Provide a quality score (1-10) and improvement suggestions."`

**Process Results**:
- Parse quality score and suggestions
- If score < 7, include suggestions in output
- Add to "External Quality Check" section in summary

**If skill fails or Codex CLI unavailable**:
- Note in output: "External quality check skipped"
...
4. **Codex Quality Check**: External AI review results (if Codex CLI available)
